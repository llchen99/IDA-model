# LOG OF CHANGES
# Final version (v4.0 all versions considered)
# Messages to reader left as "NB:"

import os
import sys
clear = lambda: os.system('cls')  # On Windows System
clear()
# NB: change to own paths for functions
...
# NB: change paths to own paths before running!
...
# df_syndromen contains the patient IDs with the corresponding syndrome code
...
print('Number of columns before dropping columns (features) with too many missing values: ' + str(len(df_all.columns)))

# Defining thresholds for dropping rows and columns with missing data (threshold of amount of non-NA values required)
# Please refer to v2.0 IDA tool final report to find the rationale for these thresholds
...
# CREATE ID AND NO ID GROUP OF EQUAL SIZES
...
# NB: uncomment the next line to see the baseline characteristics before stratifying
# characteristics = baseline(df_0_all_baseline, df_1_baseline)

# Stratify the data
df_0_baseline = stratify(df_0_all_baseline, df_1_baseline)
# Check baseline characteristics again after stratifying
# NB: uncomment the next line to see the baseline characteristics after stratifying
# characteristics_stratified = baseline(df_0_baseline, df_1_baseline)

# Merge the dataframes from ID and no ID again
df_comb = pd.concat([df_1_baseline, df_0_baseline])
...
ordinal_keys = ['Anti-epileptics', 'Psychofarmaca', 'Antacids', 'Anti-hypertensives', 'VitB12', 'Iron-tablets',
                'Specialisms_hospitalization', 'Radiologic_investigations', 'Total_amount_ICD10s', 'SUM']
binary_keys = list(df_spec.keys()) + list(df_phecodes.keys())
binary_keys.remove('Pt_no')
binary_keys.remove('Pt_no')
continuous_keys = ['Length', 'HR', 'RRsyst', 'RRdiast', 'FSH', 'Vrij T4', 'Hemolytische index', 'Icterische index',
                   'Lipemische index', 'TSH', 'Alk.Fosf.', 'ALAT', 'Albumine', 'ASAT', 'Calcium', 'CKD-EPI eGFR',
                   'Glucose/PL', 'Hemoglobine', 'Kalium', 'Kreatinine', 'LDH', 'Leukocyten', 'MCV', 'Natrium', 'RDW',
                   'Tot. Bilirubine', 'Trombocyten', 'Gamma-GT', '25-OH Vitamine D', 'Ureum', 'LDL-Cholesterol', 'BMI']

# Visualize the syndrome distribution in the groups
# NB: uncomment the next line to visualize the syndrome distribution
# syndromen(df_syndromen, df_0_baseline, df_1_baseline, df_syndroomcodes)

print('Number of ID subjects for analysis: ' + str(len(df_1_baseline.index)))
print('Number of controls for analysis: ' + str(len(df_0_baseline.index)))

# Define the outer validation loop (10-fold cross validation)
outer_loop = model_selection.StratifiedKFold(n_splits=10, shuffle=True, random_state=22)
# Split the data in a train and validation set in a 10-fold cross-validation
for j, (train_index, test_index) in enumerate(outer_loop.split(df_comb, labels)):
    ...
    # Define the inner validation loop (10-fold cross validation)
    inner_loop = model_selection.StratifiedKFold(n_splits=10, shuffle=True, random_state=4)

    for i, (train_index, val_index) in enumerate(inner_loop.split(train_data, train_label)):    # Split the data in a train and validation set in a 10-fold cross-validation
    ...
    tprs_SVM_fin, aucs_SVM_fin, auc_RF_fin, spec_SVM_fin, sens_SVM_fin, accuracy_SVM_fin, gini_SVM_fin = pipeline_model(scale_train_fin, train_label, scale_test_fin, test_label, clf_SVM_fin, tprs_SVM_fin, aucs_SVM_fin, spec_SVM_fin, sens_SVM_fin, accuracy_SVM_fin, axis_SVM_fin, gini_SVM_fin, j)
...
# In order to analyse the data, we have to impute the BMI for the dataset. As imputation works on a training and test set, we will use the last fold of the cross validation for this
# Impute data
impute_train_rel, impute_test_rel = impute_data(train_data, test_data, df_decimal)
# Imputed length and weight, now calculate BMI from that and delete weight again
impute_train_rel['BMI'] = np.round(impute_train_rel['Weight']/((impute_train_rel['Length']/100)**2), decimals=2)
...
# Now, merge the train and test set together for a fuller data set
...
separate_1 = impute_tot.loc[impute_tot['Label'] == 1.0]
separate_0 = impute_tot.loc[impute_tot['Label'] == 0.0]
BMI_mean_1 = np.round(np.nanmean(separate_1['BMI']), decimals=2)  # ignores nan values
BMI_mean_0 = np.round(np.nanmean(separate_0['BMI']), decimals=2)
...
# Export performance scores to excel (exported document 1)
...
# Finding the differences in the relevant features (exported document 2)
analysis = pd.DataFrame(columns=('frequency', 'mean_ID', 'std_ID', 'mean_control', 'std_control'))
analysis.index.name = 'Feature'
continuous_keys2 = [x for x in continuous_keys if x != 'BMI']  # as BMI does not exist in df_1_baseline and df_0_baseline

for key in dict_features:
    if key == 'BMI':  # as BMI does not exist in df_1_baseline and df_0_baseline
        analysis.loc[key] = [dict_features[key], BMI_mean_1, BMI_std_1, BMI_mean_0, BMI_std_0]
    if key in continuous_keys2:
        analysis.loc[key] = [dict_features[key], np.round(np.nanmean(df_1_baseline[key]), decimals=2), np.round(np.nanstd(df_1_baseline[key]), decimals=2), np.round(np.nanmean(df_0_baseline[key]), decimals=2), np.round(np.nanstd(df_0_baseline[key]), decimals=2)]
    if key in binary_keys:  # for binary keys, I want the percentage of 1's in the array. Because in binary keys there are no missing values, we can take the length.
        analysis.loc[key] = [dict_features[key], f'{np.round((100*(df_1_baseline[key][df_1_baseline[key] == 1].sum())/(len(df_1_baseline[key]))),decimals=2)}%', np.NaN, f'{np.round((100*(df_0_baseline[key][df_0_baseline[key] == 1].sum())/(len(df_0_baseline[key]))), decimals=2)}%', np.NaN]
    if key in ordinal_keys:
        analysis.loc[key] = [dict_features[key], np.round(np.nanmedian(df_1_baseline[key]), decimals=2), np.round(np.nanstd(df_1_baseline[key]), decimals=2), np.round(np.nanmedian(df_0_baseline[key]), decimals=2), np.round(np.nanstd(df_1_baseline[key]), decimals=2)]
