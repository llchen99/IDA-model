...
# Voor RF
def pipeline_model3(train_data, train_label, test_data, test_label, clf, parameters, tprs, aucs, spec, sens, accuracy, axis, C_sel, gamma_sel, kernel_sel):
    ...
    clf_par = RandomizedSearchCV(clf, parameters)
    clf_par.fit(train_data, train_label)
    predicted = clf_par.predict(test_data)

    # Append the chosen hyperparameters
    C_sel.append(clf_par.best_estimator_.C)
    gamma_sel.append(clf_par.best_estimator_.gamma)
    kernel_sel.append(clf_par.best_estimator_.kernel)

    # plot ROC-curve per fold
    mean_fpr = np.linspace(0, 1, 100)    # Help for plotting the false positive rate
    viz = metrics.plot_roc_curve(clf_par, test_data, test_label, name='ROC fold {}'.format(1), alpha=0.3, color = "navy", lw=1, ax=axis)    # Plot the ROC-curve for this fold on the specified axis.
    ...
    return tprs, aucs, auc, spec, sens, accuracy, gini, C_sel, gamma_sel, kernel_sel
